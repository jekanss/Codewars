/*
 An isogram is a word that has no repeating letters, consecutive or non-consecutive. Implement a function
 that determines whether a string that contains only letters is an isogram. Assume 
 the empty string is an isogram. Ignore letter case.
*/

/*
  *"Dermatoglyphics" --> true
  *"aba" --> false
  *"moOse" --> false (ignore letter case)
*/ 

export function isIsogram( str: string): any{
    //...
    const arrayStr = str.toLowerCase().split('');
    const uniqueLetters = Array.from(new Set(arrayStr));

    if (arrayStr.length === uniqueLetters.length){
        return true
    } else {
        return false
    }
    
}




