...
export function number(busStops: [number, number][]): any {;

    let pasajerosQueSuben = 0;
    let pasajerosQueBajan = 0;
    let pasajerosDespuesUltimaParada = 0;

    busStops.forEach( parada => {
        pasajerosQueSuben += parada[0];	
        pasajerosQueBajan += parada[1];	
    })

    pasajerosDespuesUltimaParada = pasajerosQueSuben - pasajerosQueBajan;

    return pasajerosDespuesUltimaParada
